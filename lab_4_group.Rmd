...
The VIF is low (<<4) for all variables in model 2 except for 'mage' and 'magesq' whose VIF was north of 80.  The correlation between these two variables is extremely high (0.99), which results in a large standard error for their coefficients.  This suggests multicollinearity is present in this model. However, this does not bias results, and does not affect interpretation of the coefficients for the main independent variables of interest, 'logvis_mo' and 'logvis_mo_sq'. \
...
<!-- Andrew's Edit Proposed Changes -->
  + Using independent variables that are calculated from the dependent variable is a violation of the linearity assumption. 

<!-- Andrew's Edit

Taking the below comment out. Via the T-Stats it does enhance the predictive power of the model, but is problematic for the linearity of the model.
...
The VIF values for the new variables are all around 1 and thus do not indicate multicollinearity. For all other variables, please refer to the discussion from Model 2. 
```{r, echo=FALSE}
vif(model_3b)
```
  
Next we look at the Breusch-Pagan test.  
```{r, echo=FALSE}
bptest(model_3)
```
The Breusch-Pagan test is significant at any level so we can reject the null hypothesis of constant variance. While the low p-value may be due to the large sample size, the level of significance indicates that we should use the 
heteroskedasticity-robust Huber-White standard errors.

Evaluating the AIC. The AIC for Model 3 is the lowest of the 3 models thus far, indiciting an improvement of fit over the prior 2 models. \
```{r, echo=FALSE}
AIC(model_3)
```

Lastly, we evaluate the diagnostic plots to check the remainder of the assumptions.  \
```{r, echo=FALSE}
plot(model_3)
```
